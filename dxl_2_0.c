...
void uart_transmit_packet(UART_HandleTypeDef *huart, uint8_t *data, uint16_t size)
{
	...
	else if(huart->Instance == USART2 && uart_tx_busy == 0)
	{
		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET); // UART2 전송 시작 표시
		HAL_UART_Transmit_DMA(huart, data, size);
		uart_tx_busy = true;
	}
	else if(huart->Instance == USART3 && uart_tx_busy == 0)
	{
		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_12, GPIO_PIN_SET); // UART3 전송 시작 표시
		HAL_UART_Transmit_DMA(huart, data, size);
		uart_tx_busy = true;
	}
	else if(huart->Instance == UART4 && uart_tx_busy == 0)
	{
		// UART4 처리 (GPIO 핀 필요한 경우 추가)
		HAL_UART_Transmit_DMA(huart, data, size);
		uart_tx_busy = true;
	}
	else if(huart->Instance == UART5 && uart_tx_busy == 0)
	{
...
void send_sync_write_1(uint8_t send_leg_type, uint16_t addr, uint16_t data_len, uint16_t *ankle_data, uint16_t *wheel_data)
{
	...
	if(send_leg_type == 0)	{
		for(int i = 1; i <= leg_count; i++) {
			...
			data_array[2*(i-1)+1] = clc_speed_1(wheel_data[i-1]); // knee 모터 골위치 또는 토크 또는 LED (1번째 요소)
		}
	}
	else if(send_leg_type == 40)	{
		for(int i = 1; i <= leg_count; i++) {
		...
		for(int i = 1; i <= leg_count; i++) {
			...
			data_array[i-1] = clc_speed_1(wheel_data[i-1]); // knee 모터 골위치 또는 토크 또는 LED (1번째 요소)
		}
	}
	else if(send_leg_type >= 1 && send_leg_type <= 4) // 특정 다리
	{
		...
		data_array[1] = clc_speed_1(wheel_data[0]); // wheel 모터 골위치 또는 토크 또는 LED (1번째 요소)
	}
	...
	else if(send_leg_type >= 51 && send_leg_type <= 54) // 특정 다리
	{
		...
		data_array[0] = clc_speed_1(wheel_data[0]);  // wheel 모터 설정 골위치 또는 토크 또는 LED (0번째 요소)
	}
